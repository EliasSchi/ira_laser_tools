...
docker run -it --rm \
    ...
    $CONTAINER_NAME \
    bash -c "rm -rf build log install && colcon build --packages-select ira_laser_tools --symlink-install \
    --cmake-args=-DCMAKE_BUILD_TYPE=Release --cmake-args -DPython3_EXECUTABLE="/usr/bin/python" --parallel-workers $(nproc)"
